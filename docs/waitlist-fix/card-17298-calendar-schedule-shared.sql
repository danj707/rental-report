...
--          with COALESCE(COALESCE(s.waitlist_config, sec.waitlist_config) ->> 'mode', 'off') <> 'off' (2 occurrences); no other changes.
-- Date: 2026-08-03
WITH cfg AS (
    SELECT
...
LEFT JOIN class_activities ca_agg ON ca_agg.class_id = sec.class_id
