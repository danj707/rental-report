...
// Guards the one remaining unbounded, org-authored string on the sheet: the
// add-on list. A 40-date permit can carry a long one, and the sheet has a fixed
// height, so it is trimmed on a word boundary rather than left to overflow.
//
// The permit's conditions of use used to be here too and are deliberately gone.
// They are terms for the holder to have agreed to, not something a crew walking
// up to a field needs, and Pawnee's run five numbered paragraphs. They live on
// the permit the QR opens.
...
// All calendar days an occurrence touches, inclusive of both ends.
function daysOf(o) {
  const out = [];
  const start = new Date(`${o.d}T00:00:00Z`), end = new Date(`${o.ed}T00:00:00Z`);
  if (isNaN(start) || isNaN(end) || end < start) return [o.d];
  for (let t = start; t <= end; t.setUTCDate(t.getUTCDate() + 1)) out.push(t.toISOString().slice(0, 10));
  return out;
}
...
function buildSchedule(s) {
  ...
  // `ed` arrived with nightly sites; treat its absence as a same-day booking so
  // a stale cached row can never render as a zero-length stay.
  rows = rows.map(r => ({ ...r, d: String(r.d), ed: String(r.ed || r.d) }));
  ...
  const covered = Array.from(new Set(use.flatMap(daysOf))).sort();
  if (covered.length < 2) return null;
  const withYear = new Set(covered.map(d => d.slice(0, 4))).size > 1;
  const span = dateSpan(covered[0], covered[covered.length - 1]);

  // ONE occurrence covering several days is a stay, not a schedule: a campsite
  // booked 9/15 3:00pm to 9/17 11:00am. Listing "Sep 15 · Sep 16 · Sep 17"
  // would be technically true and useless — what a crew and the camper both
  // need is when it starts and when the site has to be clear.
  if (use.length === 1 && use[0].ed > use[0].d) {
    const o = use[0];
    return {
      mode: "stay", count: covered.length, span, withYear,
      checkIn:  { day: shortDate(o.d,  withYear), time: timeRange(o.s, "") },
      checkOut: { day: shortDate(o.ed, withYear), time: timeRange(o.e, "") },
      groups: [], dropped: 0, commonTime: "",
    };
  }
  ...
  for (const r of use) {
    ...
    if (!groups.has(key)) {
      groups.set(key, { time: timeRange(r.s, r.e), site: oneSite ? "" : String(r.site || ""), spans: [] });
    }
    const g = groups.get(key);
    const label = `${r.d}|${r.ed}`;
    if (!g.spans.some(x => `${x.d}|${x.ed}` === label)) g.spans.push({ d: r.d, ed: r.ed });
  }
  const list = Array.from(groups.values());
  list.forEach(g => g.spans.sort((a, b) => (a.d < b.d ? -1 : a.d > b.d ? 1 : 0)));
  list.sort((a, b) => (a.spans[0].d < b.spans[0].d ? -1 : a.spans[0].d > b.spans[0].d ? 1 : 0));
  ...
  const out = [];
  for (const g of list) {
    const take = g.spans.slice(0, Math.max(budget, 0));
    dropped += g.spans.length - take.length;
    budget -= take.length;
    if (take.length) out.push({ time: g.time, site: g.site, spans: take });
  }

  return {
    mode: "list", count: covered.length, span, withYear,
...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Nightly sites: the two facts a camper and a crew both need. */
  .inout { display:flex; gap:34px; margin-top:8px; }
  .io .k   { font-size:10px; font-weight:700; letter-spacing:.16em; color:#555; }
  .io .v   { font-size:34px; font-weight:800; letter-spacing:-.02em; line-height:1.12; }
  .io .dow { font-size:15px; font-weight:600; color:#333; }
...
function datesBlock(run) {
  const chip = sp => sp.ed > sp.d
    ? `${shortDate(sp.d, run.withYear)} \u2013 ${shortDate(sp.ed, run.withYear)}`
    : shortDate(sp.d, run.withYear);
  const rows = run.groups.map(g => {
    const lead = [g.time, g.site].filter(Boolean).join(" \u00b7 ");
    const list = g.spans.map(sp => esc(chip(sp))).join(" &nbsp;&middot;&nbsp; ");
  ...
  return '<div class="dates">'
    + '<div class="label">APPROVED DATES</div>'
    + rows
    + (run.dropped
...
function sheet(s, qr) {
  ...
  // Three shapes, and the sheet has to tell them apart:
  //   single day   the row's own day, as big as it will go
  //   stay         one booking spanning days — check-in and check-out
  //   recurring    many dates, carried in the block below the fold
  const dateLine = run
    ? `${run.count} DAY${run.count === 1 ? "" : "S"} &middot; ${esc(run.span)}`
    : esc(longDate(s.date));
  const timeLine = run
  ...
  const inOut = run && run.mode === "stay"
    ? '<div class="inout">'
        + `<div class="io"><div class="k">CHECK-IN</div><div class="v">${esc(run.checkIn.time)}</div>`
        + `<div class="dow">${esc(run.checkIn.day)}</div></div>`
        + `<div class="io"><div class="k">CHECK-OUT</div><div class="v">${esc(run.checkOut.time)}</div>`
        + `<div class="dow">${esc(run.checkOut.day)}</div></div>`
      + '</div>'
    : "";
  ...
  // The tighter type is there to make room for the dates block, so only a list
  // sheet needs it. A stay has no list and can keep the full-size headline.
  return `<div class="sheet${run && run.mode === "list" ? " multi" : ""}">`
    + '<div class="top">'
      + '<div class="brand">'
    ...
    + '<div class="body">'
      + '<div class="left">'
        ...
        + '<div class="when">'
          ...
          + (inOut || `<div class="${timeCls}">${timeLine}</div>`)
        + '</div>'
        + '<div class="meta">'
          ...
          + (s.purpose ? `<b>Purpose:</b> ${esc(s.purpose)}` : "")
        + '</div>'
      + '</div>'
      // No QR, no column. An org can switch the code off (report setting
      // `permitQr`) because the public permit page it opens shows the holder's
      // details to anyone who scans a sheet on a post; the left column then
      // takes the whole width rather than leaving an empty box.
      + (qr ? '<div class="right">'
        + '<div class="qrbox">'
      ...
      + '</div>' : '')
    + '</div>'
    + (run && run.mode === "list" ? datesBlock(run) : "")
...
// ── Site tags (report setting permitLayout: "siteTag") ───────────────────────
//
// Douglas County built this themselves (Topaz_Lake_Site_Tags_Sep_18_2026.pdf)
// and asked for ours to match it: ONE TAG PER SITE, not per permit, two to a
// Letter sheet with a cut line, so a crew walks the loop with a stack of half
// sheets and clips one to each post. A site that turns over inside the window
// lists every stay on it under MULTIPLE RESERVATIONS — that is the "next
// upcoming rental under the current one" the county asked for on the call,
// answered the way their own mockup answers it.
//
// No phone, no email, no permit code: a tag on a post is public. The QR is
// printed only when the org keeps permitQr on AND the site holds one stay — two
// stays cannot share one permit link.

// "2026-09-18","2026-09-20" → "Sep 18 - Sep 20, 2026"; both years when it
// crosses one. Hand-parsed for the longDate reason.
function tagSpan(a, b) {
  const p = iso => /^(\d{4})-(\d{2})-(\d{2})/.exec(String(iso || ""));
  const x = p(a), y = p(b || a);
  if (!x) return String(a || "");
  const md = m => `${MON3[+m[2] - 1]} ${+m[3]}`;
  if (!y || (x[1] === y[1] && x[2] === y[2] && x[3] === y[3])) return `${md(x)}, ${x[1]}`;
  return x[1] === y[1] ? `${md(x)} - ${md(y)}, ${y[1]}` : `${md(x)}, ${x[1]} - ${md(y)}, ${y[1]}`;
}

// "Site 01" → { label: "SITE", big: "1" }. Anything that is not "Site <n>" is
// printed whole, smaller, with no SITE label — a pavilion's name is its number.
function tagSiteParts(site) {
  const m = /^\s*site\s*#?\s*0*(\d+[a-z]?)\s*$/i.exec(String(site || ""));
  return m ? { label: "SITE", big: m[1], name: false } : { label: "", big: String(site || ""), name: true };
}

const MAX_TAG_STAYS = 6;

const TAG_CSS = `
  @page { size: Letter portrait; margin: 0.4in; }
  * { box-sizing: border-box; }
  body { font-family: Helvetica, Arial, sans-serif; color:#000; margin:0; }
  .page { height: 10.1in; display:flex; flex-direction:column; justify-content:space-between;
          page-break-after: always; break-after: page; }
  .page:last-child { page-break-after: auto; break-after: auto; }
  .tag { height: 4.8in; border: 1.5px solid #000; padding: 0.12in 0.25in 0.14in;
         display:flex; flex-direction:column; align-items:center; text-align:center; overflow:hidden; }
  .tag.blank { border-color: transparent; }
  .t-area { font-size: 14px; font-weight: 700; letter-spacing: .01em; }
  .t-res  { font-size: 38px; font-weight: 800; line-height: 1.05; margin-top: 2px; }
  .t-lbl  { font-size: 15px; font-weight: 700; margin-top: 4px; }
  .t-num  { font-size: 58px; font-weight: 800; line-height: 1; }
  .t-num.name { font-size: 30px; line-height: 1.1; margin-top: 6px; }
  .t-rule { width: 100%; border-top: 1px solid #000; margin: 8px 0 12px; }
  .t-for  { font-size: 11px; }
  .t-who  { font-size: 28px; font-weight: 700; margin-top: 4px; }
  .t-when { font-size: 18px; font-weight: 700; margin-top: 12px; }
  .t-multi { font-size: 12px; font-weight: 700; }
  .t-list { width: 100%; margin-top: 8px; }
  .t-row  { display:flex; justify-content:space-between; align-items:baseline; gap:12px;
            padding: 0 6px; margin: 12px 0 18px; text-align:left; }
  .t-row b { font-size: 17px; }
  .t-row span { font-size: 11px; white-space: nowrap; }
  .t-more { font-size: 11px; font-style: italic; }
  .t-qr { width: 0.95in; margin-top: 8px; }
  .t-foot { margin-top: auto; font-size: 9px; font-style: italic; }
  .cut { position: relative; border-top: 1px dashed #000; height: 0; }
  .cut span { position:absolute; left:50%; top:-12px; transform:translateX(-50%);
              background:#fff; padding:0 6px; font-size:8px; }
`;

function tag(t) {
  if (!t) return '<div class="tag blank"></div>';
  const sp = tagSiteParts(t.site);
  const stays = (t.reservations || []).slice(0, MAX_TAG_STAYS);
  const more = (t.reservations || []).length - stays.length;
  const one = (t.reservations || []).length === 1;
  const body = one
    ? '<div class="t-for">Reserved for</div>'
      + `<div class="t-who">${esc(stays[0].name)}</div>`
      + `<div class="t-when">${esc(tagSpan(stays[0].start, stays[0].end))}</div>`
      + (t.qr ? `<img class="t-qr" src="${t.qr}" alt="Permit QR code" />` : "")
    : '<div class="t-multi">MULTIPLE RESERVATIONS</div><div class="t-list">'
      + stays.map(r => `<div class="t-row"><b>${esc(r.name)}</b><span>${esc(tagSpan(r.start, r.end))}</span></div>`).join("")
      + (more > 0 ? `<div class="t-more">+ ${more} more reservation${more === 1 ? "" : "s"}</div>` : "")
      + '</div>';
  return '<div class="tag">'
    + (t.area ? `<div class="t-area">${esc(String(t.area).toUpperCase())}</div>` : "")
    + '<div class="t-res">RESERVED</div>'
    + (sp.label ? `<div class="t-lbl">${sp.label}</div>` : "")
    + `<div class="t-num${sp.name ? " name" : ""}">${esc(sp.big)}</div>`
    + '<div class="t-rule"></div>'
    + body
    + '<div class="t-foot">Please leave this reservation sign in place.</div>'
  + '</div>';
}

// tags: [{area, site, reservations:[{name,start,end}], qr}], already in the
// order they should print. Two per page; an odd last tag leaves the bottom half
// blank rather than stretching.
function tagsToHtml(tags) {
  const list = tags || [];
  const pages = [];
  for (let i = 0; i < list.length; i += 2) {
    pages.push('<div class="page">' + tag(list[i])
      + '<div class="cut"><span>CUT HERE</span></div>'
      + tag(list[i + 1] || null) + '</div>');
  }
  return '<!doctype html><html><head><meta charset="utf-8"><title>Site tags</title>'
    + `<style>${TAG_CSS}</style></head><body>` + pages.join("") + '</body></html>';
}

module.exports = { toHtml, sheet, longDate, shortDate, dateSpan, timeRange, buildSchedule, esc, clamp,
                   tagsToHtml, tag, tagSpan, tagSiteParts, MAX_TAG_STAYS };
