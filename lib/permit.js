...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  /* The org mark, centred in the code. The QR is generated at error-correction
     level H (30% recoverable), and this overlay covers well under that, so the
     code still resolves to the same permit URL — verified by decoding the
     composited image, not by eye. The white plate is what makes the boundary
     read cleanly to a phone camera; without it the logo's own transparency
     leaves QR modules showing through the mark. */
  .qrlogo { position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);
            background:#fff; padding:5px 7px; border-radius:4px;
            width:32%; display:flex; align-items:center; justify-content:center; }
  .qrlogo img { width:100%; height:auto; display:block; }
      ...
      + '<div class="right">'
        + '<div class="qrbox">'
          + '<div class="qrcode">'
            ...
            + (s.logo ? `<span class="qrlogo"><img src="${s.logo}" alt="" /></span>` : "")
          + '</div>' 
