...
function dAdaptiveProfile(ctx) {
  ...
  // its cancelled count, and contributes no enrolments or capacity — it never
  ...
  return finding({
    ...
    headline: plural(groups.length, "program") + " across " + plural(secs.length, "section")
      + (participants != null ? " · " + plural(participants, "participant") + " enrolled"
         : people > 0 ? " · " + plural(round(people), "enrolment") : ""),
...
function dAdaptiveSubsidy(ctx) {
  ...
  return finding({
    ...
    basis: "Median revenue charged per enrolled participant, adaptive sections against every other section that ran in this window. Sections with no enrolments carry no per-head price and are excluded from both sides. Total net collected on adaptive in the window: " + fmtUsd(net) + ".",
    items: [{
...
// The audience. Households whose ONLY enrolment with you is adaptive — the
...
function dAdaptiveOnlyHouseholds(ctx) {
  ...
  return finding({
    ...
    basis: "Households with at least one adaptive enrolment and zero enrolments in anything else, from the participant demographics feed. Household grain because a parent registers and a child attends.",
    items: [{
...
function dAgeGap(ctx) {
  ...
  return finding({
    ...
    segment: segment("supported", under.map(r =>
      ...
      "That reaches everyone of that age on your books. It does NOT exclude the ones who already enrol — there is no \u201Chas never enrolled\u201D filter — so the CSV here is the narrower list.",
    ])),
...
function dZipGap(ctx) {
  ...
  return finding({
    ...
    basis: "Households (by their head-of-household zip code) that appear in your community records but in no enrolment. A household that rents a field or buys a pass counts as registered but not enrolled — which is the point.",
    items: rows.map(r => ({
      label: r.zip,
...
const FAMILIES = [
  ...
  { key: "adaptive",   label: "Adaptive & inclusive", emoji: "\u{267F}", blurb: "The programming built for participants with disabilities" },
